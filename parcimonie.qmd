...
```{r}
lx_pratchet <- pratchet(lx_phy, trace = 0, minit = 1000, k = 100, method = "sankoff", all = TRUE)
lx_pratchet
...
```{r}
lx_pratchet <- root(lx_pratchet, "Violet", resolve.root = TRUE, edgelabel = TRUE)
plot(lx_pratchet)
```
...
lx_phy2 <- lx_phy %>% 
  ...
  MatrixToPhyDat()
lx_pratchet <- pratchet(lx_phy2, trace = 0, minit = 1000, k = 100, method = "sankoff", all = TRUE) %>% 
  acctran(lx_phy2) %>% 
  root("Fictif", resolve.root = TRUE, edgelabel = TRUE)
lx_pratchet
lx_pratchet[[1]]
plot(lx_pratchet)
```
