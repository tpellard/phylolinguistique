...
Commençons par importer de nouveau la matrice de traits binaires que nous avions sauvegardée [précédemment](import.qmd#lst:save).

```{r}
library(tidyverse)
library(phangorn)
library(TreeTools)
lx_phy <- ReadAsPhyDat("lx_phy_bin.nex")
```
...
::: callout-caution
## À faire
Vérifier le traitement des données manquantes et celui despolymorphismes pour les données à états multiples.
:::
:::
