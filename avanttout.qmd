...
Le logiciel [R](https://www.r-project.org/), en plus d'être libre, gratuit et multi-plateforme, est particulièrement indiqué pour ce genre de travail. Il est particulièrement adapté à l'analyse quantitative et la visualisation de données et il existe de nombreuses extensions spécialisées pour la phylogénétique.
...
Les extension `ggtree` et `tanggle` pour la visualisation avancée des arbres phylogénétiques nécessitent au préalable d'installer `BiocManager`.
```{r}
