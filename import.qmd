...
Il faut donc transposer la colonne `COGIDS` en lignes (ce serait la colonne `CONCEPT` si l'on voulait obtenir une matrice de caractères à états multiples) avec la fonction `pivot_wider()`. On obtient ainsi un tableau au format large avec les langues (taxons) en colonnes et les classes de cognats (caractères) en lignes.  
