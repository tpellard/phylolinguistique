...
Il est cependant plus courant en phylolinguistique de produire une matrice avec des caractères binaires: la langue possède-t-elle (`1`) ou non (`0`) un cognat pour une classe de cognats et un concept donnés? Cela permet de traiter sans problème les cas de polymorphisme, comme on peut en trouver dans nos données d'exemple, et lève toute limite au nombre d'états possibles pour un caractère.
