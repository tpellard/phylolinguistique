...
L'indice de rétention (*retension index*, RI) est lui une mesure du degré de synapomorphie. Il se calcule:
...
L'indice de cohérence (*consistency index*, CI) est une mesure du degré d'homoplasie. Il se calcule comme le rapport du nombre de pas (changements d'état) d'un arbre sur le nombre de pas minimal nécessaire pour rendre compte des données. Plus le score est bas, plus l'arbre contient d'homoplasie, et une valeur de `1` indique une absence totale d'homoplasie.
...
L'index de cohérence redimensionné (*rescaled consistency index*, RC) est simplement le produit du CI et du RI.

```{r}
CI(lx_bab, lx_phy) * RI(lx_bab, lx_phy)
```

<!-- Le taux d'excès d'homoplasie -->
