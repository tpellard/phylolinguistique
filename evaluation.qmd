...
plotBS(lx_pratchet, p = .7)
```


## Résumer l'accord entre des résultats

Une analyse phylogénétique résulte souvent non pas en un arbre unique mais en un ensemble d'arbres. Comme ce n'est pas le cas de nos exemples jusqu'ici, générons donc un ensemble de 10 arbres aléatoires.
...
```{r}
ri <- RI(lx_bab, lx_phy, sitewise = TRUE)
ri
RI <- RI(lx_bab, lx_phy)
RI
1 - D
