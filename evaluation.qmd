...
```{r}
lx_pratchet <- read_rds("lx_pratchet.rds")
plotBS(lx_pratchet[[1]], p = .7)
```
...
C'est le cas avec les résultats de notre [analyse de parcimonie](parcimonie.qmd).
...
Un arbre consensus strict ne contient que les clades présents dans *tous* les arbres (*p* = 1), ce qui peut parfois aboutir à un «râteau» sans aucun clade à l'intérieur de l'arbre.
...
lx_sc <- consensus(lx_pratchet, p = 1)
plot(lx_sc)
```
...
```{r}
lx_mc <- consensus(lx_pratchet, p = .5)
plot(lx_mc)
```
...
Les résultats sont similaires, mais le 4^e^ arbre obtenu par parcimonie contient moins d'homoplasies et a des meilleurs scores de CI, RCI et RI. On remarque donc que l'option `all = TRUE` de `pratchet()` donne un ensemble d'arbres qui ne sont pas tous aussi parcimonieux les uns que les autres.
