...
On peut alors lancer la procédure de rééchantillonage sur l'arbre obtenu par UPGMA et la matrice de données. Même avec 1000 rééchantillonages, on obtient les résultats en un temps très court.
...
On peut évidemment de manière similaire utiliser le *bootstrap* avec la méthode du maximum de parcimonie.

```{r}
#| cache: true
lx_bab <- read.tree("lx_bab.tree")
boot_bab <- function(x) {
  bab(MatrixToPhyDat(x))
}
lx_bab_bs <- boot.phylo(lx_bab,
  lx_m,
  boot_bab,
  B = 100,
  trees = TRUE,
...
plotBS(lx_bab, lx_bab_bs$trees, p = 70)
```
