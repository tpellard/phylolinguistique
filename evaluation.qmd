...
```{r}
delta.score2(lx_phy, "all")
delta.score2(lx_phy, "mean")
delta.score2(lx_phy, "all") %>% mean()
delta.score2(lx_phy, "all") %>% 
  enframe() %>% 
  ggplot(aes(x = value)) +
...
Un score alternatif calculé comme $(m_1 - m_2)^2$ a également été proposé. On peut reprendre le code de la fonction cachée `phangorn:::delta.quartet` pour définir une nouvelle fonction à passer à notre `delta.score2()`.

```{r}
q.resid <- function (quartet, dist.dna) {
    m1 <- dist.dna[quartet[1], quartet[2]] + dist.dna[quartet[3], 
        quartet[4]]
    m2 <- dist.dna[quartet[1], quartet[3]] + dist.dna[quartet[2], 
        quartet[4]]
    m3 <- dist.dna[quartet[1], quartet[4]] + dist.dna[quartet[2], 
        quartet[3]]
    m <- sort(c(m1, m2, m3), decreasing = TRUE)
    ret <- (m[1] - m[2])^2
    return(ret)
}
delta.score2(lx_phy, "mean", f = q.resid)
delta.score2(lx_phy, "all", f = q.resid) %>% 
  enframe() %>% 
  ggplot(aes(x = value)) +
  geom_density(fill = "lightgray") +
  xlab("Q-residual") +
...
```{r}
ds <- delta.score2(lx_phy, "all")
qr <- delta.score2(lx_phy, "all", f = q.resid)
tibble(ds, qr) %>% 
  ggplot(aes(x = ds, y =qr)) +
  geom_point(color = "darkorange") +
  geom_vline(xintercept = mean(ds)) +
  geom_hline(yintercept = mean(qr)) +
  xlab("δ") +
  ylab("Q-residual") +
  theme_minimal()
```

On peut obtenir également les scores pour chaque taxon.
```{r}
...
```{r}
#| echo: false
rmarkdown::paged_table(dsqr_bytaxon)
rmarkdown::paged_table(dsqr_bytaxon_mean)
```

```{r}
dsqr_bytaxon %>% 
  ggplot(aes(x = ds, y =qr, color = taxon)) +
  geom_point() +
  geom_vline(xintercept = mean(ds)) +
  geom_hline(yintercept = mean(qr)) +
  xlab("δ") +
